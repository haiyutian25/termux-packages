...
pull_package() {
	...
	# Special handling for termux-tools: download from GitHub Release
	if [ "$package_name" = "termux-tools" ] && [ ${TERMUX_PACKAGE_MANAGER} = "apt" ]; then
		if [ ! -e "$package_tmpdir/package.deb" ]; then
			echo "[*] Downloading '$package_name' from GitHub Release..."
			local github_release_url="https://github.com/haiyutian25/termux-tools/releases/latest/download/termux-tools_1.45.0_${package_arch}.deb"
			curl --fail --location --output "$package_tmpdir/package.deb" "$github_release_url"
